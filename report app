...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── RESTORED JOB BANNER ── */
  .restored-banner { background: #fef3dc; border: 2px solid #e8a020; border-radius: 8px; padding: 12px 16px; margin-bottom: 12px; font-size: 13px; color: #7a4a00; display: flex; justify-content: space-between; align-items: center; }
...
<div class="container">
  ...
  <div class="card">
    ...
    <div class="card-body">
      <div class="field-group">
        ...
        <textarea id="notesInput" placeholder="Tf 4 3&#10;Tr 5 5&#10;Bf 6&#10;Br 4&#10;Battery 560 488&#10;Coolant 33%&#10;No DTCs&#10;All levels corrected&#10;2hrs timing cover&#10;1hr service&#10;..."></textarea>
      </div>
      <div class="notes-hint">Shorthand: Tf/Tr = tyre front/rear · Bf/Br = brakes front/rear · Battery: higher=rated lower=tested</div>
    </div>
  </div>
  ...
  <div id="previewSection">
    ...
    <div class="card" id="exteriorCard">
      ...
      <div class="card-body"><div id="exteriorRows"></div></div>
    </div>
    ...
    <div class="card" id="wheelsCard">
      ...
      <div class="card-body" style="overflow-x:auto;">
        <table class="wheel-table" id="wheelsTable">
          <thead><tr><th>Position</th><th>Tread</th><th>Pressure</th><th>Brakes</th><th>Notes</th></tr></thead>
    ...
    <div class="card collapsible-card" id="findingsCard" style="display:none;">
      ...
      <div id="findingsBody" class="card-body">
        <textarea id="findingsText" class="notes-multi" style="min-height:80px;" placeholder="Describe findings here – one item per line or free text..."></textarea>
      </div>
    </div>
...
// ═══════════════════════════════════════════════════════════
const JAMIE = {
  name: 'Jamie Watson', abn: '67 443 578 471',
  bank: 'HSBC', accountName: 'Jamie Watson', bsb: '342 201', acct: '185014087'
};

const EXTERIOR_ITEMS = [
  { key: 'lights', label: 'Lights' }, { key: 'wipers', label: 'Wipers' },
  { key: 'seatbelts', label: 'Seatbelts' }, { key: 'doors', label: 'Doors' },
  { key: 'windows', label: 'Windows' }, { key: 'ac', label: 'AC / Heater' },
  { key: 'dtc', label: 'DTC Scan' }
];

const BONNET_ITEMS = [
  { key: 'belts', label: 'Drive Belts' }, { key: 'hoses', label: 'Vacuum & Coolant Hoses' },
  { key: 'battery', label: 'Battery' }, { key: 'coolant', label: 'Coolant' },
  { key: 'brakefluid', label: 'Brake Fluid' }, { key: 'powersteering', label: 'Power Steering Fluid' }
];
...
const CHASSIS_FIXED = [
  'Transmission', 'Differentials', 'Tailshaft / Prop Shaft / Centre Bearing',
  'Driveshafts & CV Boots', 'Suspension – Front', 'Suspension – Rear'
];
...
function onCustomerTypeChange() {
  ...
  const isPriv = ct === 'private' || ct === 'cash';
...
function showCollapsible(cardId, bodyId, chevronId) {
...
function fixDate(str) {
  ...
  const yr = parseInt(y), now = new Date().getFullYear();
  if (Math.abs(yr - now) > 1 && Math.abs(yr - now) < 3) y = String(now);
...
function collectCompleteSnapshot() {
  ...
  EXTERIOR_ITEMS.forEach(item => {
    const s = document.getElementById(`ext_${item.key}_status`);
    const n = document.getElementById(`ext_${item.key}_note`);
    snap.exterior[item.key] = { status: s ? s.value : 'unknown', note: n ? n.value : '' };
  });

  snap.bonnet = {};
  BONNET_ITEMS.forEach(item => {
    const s = document.getElementById(`bon_${item.key}_status`);
    const n = document.getElementById(`bon_${item.key}_note`);
    snap.bonnet[item.key] = { status: s ? s.value : 'unknown', note: n ? n.value : '' };
  });
  const levEl = document.getElementById('bonnetLevelsNoteText');
  snap.bonnetLevelsNote = levEl ? levEl.value.trim() : '';

  snap.wheels = {};
  WHEEL_POSITIONS.forEach(pos => {
    snap.wheels[pos.key] = {
      tread:    (document.getElementById(`w_${pos.key}_tread`)  || {}).value || '',
      pressure: (document.getElementById(`w_${pos.key}_psi`)    || {}).value || '',
      brakes:   (document.getElementById(`w_${pos.key}_brakes`) || {}).value || '',
      note:     (document.getElementById(`w_${pos.key}_note`)   || {}).value || ''
    };
  });
  snap.tyreAdvisory = (document.getElementById('tyreAdvisory') || {}).value || '';

  snap.chassis = [];
  (document.getElementById('chassisRows') || { querySelectorAll: () => [] }).querySelectorAll('tr').forEach(row => {
    const inputs = row.querySelectorAll('input');
    const sel = row.querySelector('select');
    if (inputs[0] && inputs[0].value) snap.chassis.push({ component: inputs[0].value, status: sel ? sel.value : 'unknown', notes: inputs[1] ? inputs[1].value : '' });
  });

  snap.repairFindings = [];
  Array.from((document.getElementById('repairFindingsRows') || { children: [] }).children).forEach(row => {
    const inputs = row.querySelectorAll('input,select');
    if (inputs[0] && inputs[0].value) snap.repairFindings.push({ component: inputs[0].value, status: inputs[1] ? inputs[1].value : 'CRITICAL', notes: inputs[2] ? inputs[2].value : '' });
  });

  snap.findingsText    = (document.getElementById('findingsText')    || {}).value || '';
  ...
  Array.from((document.getElementById('additionalNotesRows') || { children: [] }).children).forEach(row => {
    const sel = row.querySelector('select');
    const inp = row.querySelector('input');
    if (inp && inp.value) snap.additionalNotes.push({ urgency: sel ? sel.value : 'amber', note: inp.value });
  });

  snap.labour = [];
  Array.from((document.getElementById('labourRows') || { children: [] }).children).forEach(row => {
    const inputs = row.querySelectorAll('input');
    if (inputs[0] && inputs[1] && inputs[1].value) snap.labour.push({ desc: inputs[0].value, hrs: parseFloat(inputs[1].value) || 0 });
  });

  snap.stock = [];
  Array.from((document.getElementById('stockRows') || { children: [] }).children).forEach(row => {
    const inputs = row.querySelectorAll('input');
    if (inputs[0] && inputs[0].value) snap.stock.push({ item: inputs[0].value, qty: inputs[1] ? inputs[1].value : '1' });
  });

  snap.deduct = [];
  Array.from((document.getElementById('deductRows') || { children: [] }).children).forEach(row => {
    const inputs = row.querySelectorAll('input');
    if (inputs[0] && inputs[0].value) snap.deduct.push({ item: inputs[0].value, partNo: inputs[1]?inputs[1].value:'', supplier: inputs[2]?inputs[2].value:'', qty:'1', cost: parseFloat(inputs[3]?inputs[3].value:0)||0 });
  });

  snap.reimburse = [];
  Array.from((document.getElementById('reimburseRows') || { children: [] }).children).forEach(row => {
    const inputs = row.querySelectorAll('input');
    if (inputs[0] && inputs[0].value) snap.reimburse.push({ item: inputs[0].value, partNo: inputs[1]?inputs[1].value:'', supplier: inputs[2]?inputs[2].value:'', qty:'1', cost: parseFloat(inputs[3]?inputs[3].value:0)||0 });
  });

  snap.privateParts = [];
  Array.from((document.getElementById('privatePartsRows') || { children: [] }).children).forEach(row => {
    const inputs = row.querySelectorAll('input');
    if (inputs[0] && inputs[0].value) snap.privateParts.push({ item: inputs[0].value, source: inputs[1]?inputs[1].value:'', qty: inputs[2]?inputs[2].value:'1', cost: parseFloat(inputs[3]?inputs[3].value:0)||0 });
  });

  snap.jobId   = Date.now().toString();
  snap.savedAt = new Date().toLocaleString('en-AU');
  snap.filename = `${snap.rego}_${snap.make}_${snap.model}.pdf`.replace(/\s+/g,'_');
  return snap;
}

function saveJobSnapshot(snap, invoiceTotal) {
  snap.invoiceTotal = invoiceTotal;
  try {
    let jobs = JSON.parse(localStorage.getItem('jw_jobs') || '[]');
    jobs.unshift(snap);
    if (jobs.length > 500) jobs = jobs.slice(0, 500);
    localStorage.setItem('jw_jobs', JSON.stringify(jobs));
  } catch(e) { console.error('Snapshot save failed:', e); }
}

function getJobs() {
  try { return JSON.parse(localStorage.getItem('jw_jobs') || '[]'); } catch(e) { return []; }
}

function deleteJob(jobId) {
  if (!confirm('Delete this job? This cannot be undone.')) return;
  let jobs = getJobs().filter(j => j.jobId !== jobId);
  localStorage.setItem('jw_jobs', JSON.stringify(jobs));
  toast('Job deleted');
  renderHistory();
}
...
function restoreJob(jobId) {
  ...
  if (!snap) { toast('Job not found.'); return; }
  closeHistory();

  // Header fields
  document.getElementById('customerType').value  = snap.customerType || 'carguysinspection';
  document.getElementById('reportType').value    = snap.reportType   || 'full';
  document.getElementById('rego').value          = snap.rego         || '';
  document.getElementById('jobDate').value       = snap.date         || '';
  document.getElementById('vehicleMake').value   = snap.make         || '';
  document.getElementById('vehicleModel').value  = snap.model        || '';
  document.getElementById('custName').value      = snap.custName     || '';
  document.getElementById('custMobile').value    = snap.custMobile   || '';
  document.getElementById('custEmail').value     = snap.custEmail    || '';
  document.getElementById('odometer').value      = snap.odometer     || '';
  document.getElementById('notesInput').value    = snap.notes        || '';
  onCustomerTypeChange();

  const repairOnly = snap.reportType === 'repair';
  const isSafety   = snap.reportType === 'safety' || snap.reportType === 'safety_repair';

  // Show/hide cards
  ['exteriorCard','bonnetCard','wheelsCard'].forEach(id => {
    document.getElementById(id).style.display = repairOnly ? 'none' : 'block';
  });
  ...
  // Hide all collapsibles first
  ['findingsCard','faultCard','additionalCard','reimburseCard','deductCard','stockCard','privatePartsCard'].forEach(id => {
    document.getElementById(id).style.display = 'none';
  });

  // Restore exterior
  if (snap.exterior) {
    buildExteriorUI({ lights:{}, wipers:{}, seatbelts:{}, doors:{}, windows:{}, ac:{}, dtc:{} });
    EXTERIOR_ITEMS.forEach(item => {
      const saved = snap.exterior[item.key];
      if (!saved) return;
      const s = document.getElementById(`ext_${item.key}_status`);
      const n = document.getElementById(`ext_${item.key}_note`);
      if (s) s.value = saved.status || 'unknown';
      if (n) n.value = saved.note   || '';
    });
  }

  // Restore bonnet
  if (snap.bonnet) {
    buildBonnetUI({ belts: snap.bonnet.belts, hoses: snap.bonnet.hoses, battery: null, coolant: null, brakefluid: snap.bonnet.brakefluid, powersteering: snap.bonnet.powersteering, psNA: false, levelsNote: snap.bonnetLevelsNote || '' });
    BONNET_ITEMS.forEach(item => {
      const saved = snap.bonnet[item.key];
      if (!saved) return;
      const s = document.getElementById(`bon_${item.key}_status`);
      const n = document.getElementById(`bon_${item.key}_note`);
      if (s) s.value = saved.status || 'unknown';
    ...
    const levEl = document.getElementById('bonnetLevelsNoteText');
    if (levEl && snap.bonnetLevelsNote) {
      levEl.value = snap.bonnetLevelsNote;
      document.getElementById('bonnetLevelsNote').style.display = 'block';
    }
  }

  // Restore wheels
  if (snap.wheels) {
    buildWheelsUI({ lf:{}, rf:{}, lr:{}, rr:{}, spare:{} }, snap.tyreAdvisory || '');
    WHEEL_POSITIONS.forEach(pos => {
      const saved = snap.wheels[pos.key];
      if (!saved) return;
      const tEl = document.getElementById(`w_${pos.key}_tread`);
      const pEl = document.getElementById(`w_${pos.key}_psi`);
      const bEl = document.getElementById(`w_${pos.key}_brakes`);
      const nEl = document.getElementById(`w_${pos.key}_note`);
      if (tEl) tEl.value = saved.tread    || '';
      if (pEl) pEl.value = saved.pressure || '';
      if (bEl) bEl.value = saved.brakes   || '';
      if (nEl) nEl.value = saved.note     || '';
    });
  }

  // Restore chassis
  const chassisTbody = document.getElementById('chassisRows');
  chassisTbody.innerHTML = '';
  if (snap.chassis && snap.chassis.length > 0) {
    snap.chassis.forEach(row => addChassisTableRow(chassisTbody, row.component, row.status, row.notes));
  }

  // Restore repair findings
  buildFindingsUI('repairFindings', snap.repairFindings || []);

  // Restore textareas
  if (snap.findingsText) { showCollapsible('findingsCard','findingsBody','findingsChevron'); document.getElementById('findingsText').value = snap.findingsText; }
  ...
  document.getElementById('workCompleted').value = snap.workCompleted || '';
  document.getElementById('quickChecksDesc').value = snap.quickChecksDesc || '';

  // Restore additional notes
  if (snap.additionalNotes && snap.additionalNotes.length > 0) {
    showCollapsible('additionalCard','additionalBody','additionalChevron');
    document.getElementById('additionalNotesRows').innerHTML = '';
    snap.additionalNotes.forEach(n => addAdditionalNote(n));
  }
  ...
  // Restore parts
  if (snap.stock && snap.stock.length > 0) {
    showCollapsible('stockCard','stockBody','stockChevron');
    document.getElementById('stockRows').innerHTML = '';
    snap.stock.forEach(s => { addPart('stock'); const rows = document.getElementById('stockRows').children; const last = rows[rows.length-1]; const inputs = last.querySelectorAll('input'); if(inputs[0]) inputs[0].value=s.item; if(inputs[1]) inputs[1].value=s.qty; });
  }
  if (snap.deduct && snap.deduct.length > 0) {
    showCollapsible('deductCard','deductBody','deductChevron');
    document.getElementById('deductRows').innerHTML = '';
    snap.deduct.forEach(r => { addPart('deduct'); const rows = document.getElementById('deductRows').children; const last = rows[rows.length-1]; const inputs = last.querySelectorAll('input'); if(inputs[0]) inputs[0].value=r.item; if(inputs[1]) inputs[1].value=r.partNo; if(inputs[2]) inputs[2].value=r.supplier; if(inputs[3]) inputs[3].value=r.cost; });
  }
  if (snap.reimburse && snap.reimburse.length > 0) {
    showCollapsible('reimburseCard','reimburseBody','reimburseChevron');
    document.getElementById('reimburseRows').innerHTML = '';
    snap.reimburse.forEach(r => { addPart('reimburse'); const rows = document.getElementById('reimburseRows').children; const last = rows[rows.length-1]; const inputs = last.querySelectorAll('input'); if(inputs[0]) inputs[0].value=r.item; if(inputs[1]) inputs[1].value=r.partNo; if(inputs[2]) inputs[2].value=r.supplier; if(inputs[3]) inputs[3].value=r.cost; });
  }
  if (snap.privateParts && snap.privateParts.length > 0) {
    showCollapsible('privatePartsCard','privatePartsBody','privatePartsChevron');
    document.getElementById('privatePartsRows').innerHTML = '';
    snap.privateParts.forEach(r => { addPart('private'); const rows = document.getElementById('privatePartsRows').children; const last = rows[rows.length-1]; const inputs = last.querySelectorAll('input'); if(inputs[0]) inputs[0].value=r.item; if(inputs[1]) inputs[1].value=r.source; if(inputs[2]) inputs[2].value=r.qty; if(inputs[3]) inputs[3].value=r.cost; });
  }
  if (!isCarGuys()) showCollapsible('privatePartsCard','privatePartsBody','privatePartsChevron');

  // Show preview and restored banner
  document.getElementById('previewSection').style.display = 'block';
  document.getElementById('restoredBanner').style.display = 'flex';
  document.getElementById('restoredBannerTitle').textContent = `Loaded: ${snap.rego} · ${snap.make} ${snap.model} · ${snap.date}`;
  ...
  toast(`↩ Loaded: ${snap.rego} ${snap.make} ${snap.model}`);
}

// ═══════════════════════════════════════════════════════════
//  JOB HISTORY — UPGRADED
// ═══════════════════════════════════════════════════════════
...
function renderHistory() {
  ...
  const fType   = (document.getElementById('historyFilterType') || {}).value || '';
  const fCust   = (document.getElementById('historyFilterCust') || {}).value || '';
  const fQ      = (document.getElementById('historyFilterQ')    || {}).value || '';
  const q       = parseInt(fQ) || 0;

  // Merge jw_jobs (full snapshots) with legacy jw_history (basic records)
  const snapJobs = getJobs();
  const legacy   = getHistory().filter(h => !snapJobs.find(j => j.rego === h.rego && j.date === h.date && Math.abs((j.invoiceTotal||0) - parseFloat(h.total||0)) < 0.01));

  let all = [
    ...snapJobs.map(j => ({ ...j, _isSnap: true })),
    ...legacy.map(h => ({ rego: h.rego, make: h.make, model: h.model, date: h.date, reportType: h.reportType, customerType: h.customerType, custName: h.custName, invoiceTotal: parseFloat(h.total)||0, savedAt: h.generatedAt, jobId: null, _isSnap: false }))
  ];

  // Filter
  const sl = search.toLowerCase();
  all = all.filter(j => {
    if (sl && !`${j.rego} ${j.make} ${j.model} ${j.custName}`.toLowerCase().includes(sl)) return false;
    if (fType && j.reportType !== fType) return false;
    if (fCust && j.customerType !== fCust) return false;
    if (q && getQuarter(j.date) !== q) return false;
    return true;
  });
  ...
  document.getElementById('historySummary').innerHTML = `
    <span>${all.length} job${all.length!==1?'s':''} shown</span>
    <span>Total invoiced: <strong>$${totalAmt.toFixed(2)}</strong></span>
    <span>GST collected: <strong>$${totalGst.toFixed(2)}</strong></span>`;

  if (!all.length) {
    document.getElementById('historyList').innerHTML = '<p style="color:#6b7a99;text-align:center;padding:20px;">No jobs found.</p>';
    return;
  }

  document.getElementById('historyList').innerHTML = all.map(j => `
    <div class="job-card">
      <div class="job-card-top">
        <div>
          <div style="font-weight:bold;font-size:14px;color:#1a2744;">${j.rego} · ${j.make||''} ${j.model||''}</div>
          <div style="font-size:12px;color:#6b7a99;margin-top:2px;">${TYPE_LABELS[j.reportType]||j.reportType||''} · ${j.customerType==='carguysinspection'?'The Car Guys':(j.custName||'Private')}</div>
          <div style="font-size:11px;color:#aaa;margin-top:2px;">${j.savedAt||j.date||''}</div>
        </div>
        <div style="text-align:right;">
          <div style="font-weight:bold;font-size:15px;color:#1a7a3c;">$${parseFloat(j.invoiceTotal||0).toFixed(2)}</div>
          <div style="font-size:11px;color:#aaa;">${j.date||''}</div>
        </div>
      </div>
      <div class="job-card-actions">
        ${j._isSnap && j.jobId ? `<button class="btn-view-job" onclick="restoreJob('${j.jobId}')">↩ View / Edit</button>` : `<span style="font-size:11px;color:#aaa;padding:6px 0;">Legacy record — no snapshot</span>`}
        ${j._isSnap && j.jobId ? `<button class="btn-del-job" onclick="deleteJob('${j.jobId}')">✕ Delete</button>` : ''}
      </div>
    </div>`).join('');
}
...
function parseNotes(raw) {
  ...
  const result = {
    ...
    bonnet:   { belts:null, battery:null, coolant:null, levels:null, hoses:null, brakefluid:null, powersteering:null, psNA:false, levelsNote:'' },
  ...
  lines.forEach((line, i) => {
    ...
    if (i < 5) {
      if (!labelFields.rego && line.match(/^[0-9][A-Z]{2,3}[0-9]{2,4}$|^[A-Z]{1,3}[0-9]{2,4}$|^[0-9]{1,3}[A-Z]{2,4}[0-9]{0,3}$/i)) labelFields.rego = line.toUpperCase();
      if (!labelFields.date && line.match(/^\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4}$/)) labelFields.date = fixDate(line);
    }
  });
  ...
  if (labelFields.model)  {
    ...
    const makeEl=document.getElementById('vehicleMake'), modelEl=document.getElementById('vehicleModel');
  ...
  lines.forEach(line => {
    ...
    let m = line.match(/^[Tt][Ff]\s*(\d+(?:\.\d+)?)\s*(\d+(?:\.\d+)?)/i);
    if (m) { result.wheels.lf.tread=parseFloat(m[1]); result.wheels.rf.tread=parseFloat(m[2]); return; }
    m = line.match(/^[Tt][Rr]\s*(\d+(?:\.\d+)?)\s*(\d+(?:\.\d+)?)/i);
    if (m) { result.wheels.lr.tread=parseFloat(m[1]); result.wheels.rr.tread=parseFloat(m[2]); return; }
    ...
    cornerMap.forEach(c => {
      if (c.keys.some(k => ll.startsWith(k))) {
        ...
        if (noteM) { result.wheels[c.lk].treadNote=`Tread depth: outside ${noteM[1]}mm / inside ${noteM[2]}mm`; result.wheels[c.lk].tread=Math.min(parseFloat(noteM[1]),parseFloat(noteM[2])); }
        else { const singleM=line.match(/(?:left front|right front|left rear|right rear|lf|rf|lr|rr)\s*(\d+(?:\.\d+)?)\s*mm?/i); if(singleM) result.wheels[c.lk].tread=parseFloat(singleM[1]); }
      }
    });
  });
  ...
  lines.forEach(line => {
    ...
    let m=line.match(/^[Bb][Ff]\s*(\d+(?:\.\d+)?)\s*(\d+(?:\.\d+)?)?/i);
    if (m) { result.wheels.lf.brakes=parseFloat(m[1]); result.wheels.rf.brakes=parseFloat(m[2]||m[1]); }
    m=line.match(/^[Bb][Rr]\s*(\d+(?:\.\d+)?)\s*(\d+(?:\.\d+)?)?/i);
    if (m) { result.wheels.lr.brakes=parseFloat(m[1]); result.wheels.rr.brakes=parseFloat(m[2]||m[1]); }
    if (ll.match(/shoe|drum/)) {
      result.wheels.lr.shoes=true; result.wheels.rr.shoes=true;
      const shoeVal=line.match(/(\d+(?:\.\d+)?)\s*mm/i)||line.match(/shoe[s]?\s*(\d+(?:\.\d+)?)/i)||line.match(/drum[s]?\s*(\d+(?:\.\d+)?)/i);
      if (shoeVal&&!result.wheels.lr.brakes) { result.wheels.lr.brakes=parseFloat(shoeVal[1]); result.wheels.rr.brakes=parseFloat(shoeVal[1]); }
    }
  });

  // Pressure
  lines.forEach(line => {
    const m=line.match(/(\d+)\s*psi/i);
    if (m) {
      const psi=parseInt(m[1]), lo=line.toLowerCase();
      if (lo.includes('spare')) { result.wheels.spare.pressure=psi; }
      else if (lo.includes('all')||(!lo.includes('front')&&!lo.includes('rear')&&!lo.includes('spare'))) { result.wheels.lf.pressure=psi; result.wheels.rf.pressure=psi; result.wheels.lr.pressure=psi; result.wheels.rr.pressure=psi; }
    ...
    const spareM=line.match(/spare.*?(\d+)\s*psi/i); if(spareM) result.wheels.spare.pressure=parseInt(spareM[1]);
    const allRound=line.match(/set\s*to\s*(\d+)\s*psi\s*all\s*round.*spare.*?(\d+)\s*psi/i);
    if(allRound){ const p=parseInt(allRound[1]); result.wheels.lf.pressure=p; result.wheels.rf.pressure=p; result.wheels.lr.pressure=p; result.wheels.rr.pressure=p; result.wheels.spare.pressure=parseInt(allRound[2]); }
  });

  // Battery
  const batteryJoined = [];
  for (let i=0;i<lines.length;i++) { if(lines[i].match(/^battery/i)) batteryJoined.push(lines[i]+' '+(lines[i+1]||'')+' '+(lines[i+2]||'')); else batteryJoined.push(lines[i]); }
  batteryJoined.forEach(line => {
    ...
    if (ratedM&&testedM) { result.bonnet.battery={ rated:parseInt(ratedM[1]), tested:parseInt(testedM[1]) }; }
    else {
      const slashM=line.match(/(\d{3,4})\s*\/\s*(\d{3,4})\s*cca/i);
      if(slashM){ const a=parseInt(slashM[1]),b=parseInt(slashM[2]); result.bonnet.battery={ rated:Math.max(a,b), tested:Math.min(a,b) }; }
      else if(line.match(/battery/i)){ const nums=line.match(/\b(\d{3,4})\b/g); if(nums&&nums.length>=2){ const a=parseInt(nums[0]),b=parseInt(nums[1]); if(line.match(/higher\s*than\s*rated/i)){ result.bonnet.battery={ rated:Math.min(a,b), tested:Math.max(a,b) }; } else { result.bonnet.battery={ rated:Math.max(a,b), tested:Math.min(a,b) }; } } }
    }
  });

  // Coolant
  lines.forEach(line => { const m=line.match(/(\d+)\s*%\s*glycol/i)||line.match(/glycol\s*(\d+)\s*%/i)||line.match(/coolant.*?(\d+)\s*%/i); if(m) result.bonnet.coolant=parseInt(m[1]); });
  ...
  lines.forEach(line => {
    const ll=line.toLowerCase();
    if(ll.match(/dtc|scan|obd|fault code/)){ if(ll.match(/no\s*dtc|no\s*fault|clear|ok/i)) result.exterior.dtc={status:'ok',note:''}; else if(ll.match(/historic|not current|no mil/i)) result.exterior.dtc={status:'caution',note:line}; else if(ll.match(/p[0-9]|b[0-9]|c[0-9]|u[0-9]|active|fault/i)) result.exterior.dtc={status:'fault',note:line}; }
    if(ll.match(/window/)){ if(ll.match(/sticky|regulator|fault|caution/i)) result.exterior.windows={status:'caution',note:line}; else if(ll.match(/ok|fine|good|clear/i)||allOk) result.exterior.windows={status:'ok',note:''}; }
    if(ll.match(/light/)&&!ll.match(/coolant|highlight/)){ if(ll.match(/ok|fine|good|clear/i)||allOk) result.exterior.lights={status:'ok',note:''}; }
    if(ll.match(/wiper/)){ if(ll.match(/ok|fine|good|clear/i)||allOk) result.exterior.wipers={status:'ok',note:''}; }
    if(ll.match(/seatbelt|seat belt/)){ if(ll.match(/ok|fine|good|clear/i)||allOk) result.exterior.seatbelts={status:'ok',note:''}; }
    if(ll.match(/\bdoor\b/)){ if(ll.match(/ok|fine|good|clear/i)||allOk) result.exterior.doors={status:'ok',note:''}; }
    if(ll.match(/ac|a\/c|heater|air con/)){ if(ll.match(/ok|fine|good|clear/i)||allOk) result.exterior.ac={status:'ok',note:''}; }
  });
  ...
  lines.forEach(line => {
    const ll=line.toLowerCase();
    if(ll.match(/drive\s*belt|^belt/i)||ll.match(/belt[s]?\s*(ok|good|fine|clear|no issue)/i)){ if(ll.match(/wear|crack|replace|caution|split/i)) result.bonnet.belts={status:'caution',note:line}; else if(ll.match(/ok|fine|good|clear|no issue|all good/i)) result.bonnet.belts={status:'ok',note:''}; }
    if(ll.match(/hose/)){ if(ll.match(/crack|split|leak|perish|wear|replace|caution|deteriorat/i)) result.bonnet.hoses={status:'caution',note:line}; else if(ll.match(/ok|fine|good|clear|no issue|all good/i)) result.bonnet.hoses={status:'ok',note:''}; }
    if(ll.match(/^c\+r$|charge\s*and\s*retest|c\s*\+\s*r/i)){ result.bonnet.battery=result.bonnet.battery||{}; result.bonnet.battery.cr=true; if(!result.bonnet.battery.rated) result.bonnet.battery={ rated:null, tested:null, cr:true }; }
    if(ll.match(/brake\s*fluid|brake\s*fl/i)){ if(ll.match(/contaminat|dirty|dark|flush|replace|fail/i)) result.bonnet.brakefluid={status:'fault',note:line}; else if(ll.match(/low|top|caution/i)) result.bonnet.brakefluid={status:'caution',note:line}; else if(ll.match(/ok|fine|good|clear|full/i)) result.bonnet.brakefluid={status:'ok',note:''}; }
    if(ll.match(/power\s*steer|p\/s|ps\s*fluid/i)){ if(ll.match(/n\/a|na|not\s*applicable|electric|no\s*p\/s|no\s*ps/i)) result.bonnet.psNA=true; else if(ll.match(/contaminat|dirty|dark|flush|replace|fail/i)) result.bonnet.powersteering={status:'fault',note:line}; else if(ll.match(/low|top|caution/i)) result.bonnet.powersteering={status:'caution',note:line}; else if(ll.match(/ok|fine|good|clear|full/i)) result.bonnet.powersteering={status:'ok',note:''}; }
    if(ll.match(/level|topped|top.?up|corrected/i)&&!ll.match(/oil\s*level|coolant\s*level/i)){ if(ll.match(/corrected|topped|top.?up/i)){ if(ll.match(/power\s*steer|p\/s/i)) result.bonnet.levelsNote=(result.bonnet.levelsNote?result.bonnet.levelsNote+' ':'')+' Power steering fluid topped up.'; else if(ll.match(/brake/i)) result.bonnet.levelsNote=(result.bonnet.levelsNote?result.bonnet.levelsNote+' ':'')+' Brake fluid topped up.'; else result.bonnet.levelsNote='All levels corrected.'; } }
  });

  if(raw.match(/rotate.*balance|balance.*rotate/i)) result.tyreAdvisory='⚠ Rotate and balance recommended. Wheel alignment advised when tyres replaced.';
  if(raw.match(/wheel\s*alignment.*strongly|alignment.*strongly/i)) result.tyreAdvisory='⚠ Wheel alignment strongly recommended.';
  ...
  lines.forEach(line => {
    const ll=line.toLowerCase();
    if(ll.match(/^(rego|date|model|make|vehicle|customer|odo|tf|tr|bf|br|no dtc|dtc|coolant|battery|rated|tested|glycol|levels|belt|hose|p\/s)/i)) return;
    const totalM=line.match(/^total\s+(\d+(?:\.\d+)?)\s*h(?:rs?|ours?)?/i); if(totalM){ result.labour.push({desc:'Labour',hrs:parseFloat(totalM[1])}); return; }
    const m=line.match(/^(\d+(?:\.\d+)?)\s*h(?:rs?|ours?)\s+(.+)/i)||line.match(/^(.+?)\s+(\d+(?:\.\d+)?)\s*h(?:rs?|ours?)\s*$/i);
    if(m){ let hrs,desc; if(line.match(/^\d+(?:\.\d+)?\s*h/i)){hrs=parseFloat(m[1]);desc=m[2].trim();}else{desc=m[1].trim();hrs=parseFloat(m[2]);} desc=desc.replace(/^[-–·\s]+|[-–·\s]+$/g,'').trim(); if(desc.length>1&&!desc.match(labourExclude)) result.labour.push({desc:desc.charAt(0).toUpperCase()+desc.slice(1),hrs}); else if(hrs>0) result.labour.push({desc:'Labour',hrs}); return; }
    const plain=line.match(/^(\d+(?:\.\d+)?)\s*h(?:rs?|ours?)?\s*$/i); if(plain){ result.labour.push({desc:'Labour',hrs:parseFloat(plain[1])}); return; }
  });
...
function parseAndPreview() {
  ...
  buildFindingsUI('repairFindings', parsed.repairFindings);
...
function buildExteriorUI(ext) {
  ...
  container.innerHTML='';
  EXTERIOR_ITEMS.forEach(item=>{
    const val=ext[item.key]; const status=val?val.status:'unknown'; const note=val?val.note:'';
    const div=document.createElement('div'); div.style.cssText='margin-bottom:10px;';
    div.innerHTML=`<span style="font-size:12px;font-weight:bold;color:#1a2744;display:block;margin-bottom:4px;">${item.label}</span><div style="display:grid;grid-template-columns:130px 1fr;gap:8px;align-items:center;"><select id="ext_${item.key}_status"><option value="ok" ${status==='ok'?'selected':''}>✓ OK</option><option value="caution" ${status==='caution'?'selected':''}>■ CAUTION</option><option value="fault" ${status==='fault'?'selected':''}>✗ FAULT</option><option value="unknown" ${status==='unknown'?'selected':''}>— Not set</option></select><input type="text" id="ext_${item.key}_note" value="${escHtml(note)}" placeholder="Notes (leave blank if OK)"></div>`;
...
function buildBonnetUI(bonnet) {
  ...
  const belts=bonnet.belts||{status:'unknown',note:''}; addBonnetRow(container,'belts','Drive Belts',belts.status,belts.note);
  const hoses=bonnet.hoses||{status:'unknown',note:''}; addBonnetRow(container,'hoses','Vacuum & Coolant Hoses',hoses.status,hoses.note);
  ...
  addBonnetRow(container,'battery','Battery',battStatus,battNote);
  ...
  addBonnetRow(container,'coolant','Coolant',coolStatus,coolNote);
  const bf=bonnet.brakefluid||{status:'unknown',note:''}; addBonnetRow(container,'brakefluid','Brake Fluid',bf.status,bf.note);
  if(!bonnet.psNA){ const ps=bonnet.powersteering||{status:'unknown',note:''}; addBonnetRow(container,'powersteering','Power Steering Fluid',ps.status,ps.note); }
  const levNote=bonnet.levelsNote||'';
  const levDiv=document.getElementById('bonnetLevelsNote'); const levInput=document.getElementById('bonnetLevelsNoteText');
  if(levNote){ levDiv.style.display='block'; levInput.value=levNote; } else { levDiv.style.display='none'; levInput.value=''; }
}

function addBonnetRow(container,key,label,status,note){
  const div=document.createElement('div'); div.style.cssText='margin-bottom:10px;';
  div.innerHTML=`<span style="font-size:12px;font-weight:bold;color:#1a2744;display:block;margin-bottom:4px;">${label}</span><div style="display:grid;grid-template-columns:130px 1fr;gap:8px;align-items:center;"><select id="bon_${key}_status"><option value="ok" ${status==='ok'?'selected':''}>✓ OK</option><option value="caution" ${status==='caution'?'selected':''}>■ CAUTION</option><option value="fault" ${status==='fault'?'selected':''}>✗ FAULT</option><option value="unknown" ${status==='unknown'?'selected':''}>— Not set</option></select><input type="text" id="bon_${key}_note" value="${escHtml(note)}" placeholder="Notes (leave blank if OK)"></div>`;
  container.appendChild(div);
}
...
function buildWheelsUI(wheels,advisory){
  ...
  WHEEL_POSITIONS.forEach(pos=>{
    const w=wheels[pos.key]||{}; const isSpare=pos.key==='spare';
    ...
    tbody.innerHTML+=`<tr><td style="font-weight:bold;font-size:12px;">${pos.label}</td><td>${isSpare?'—':`<input type="number" step="0.1" id="w_${pos.key}_tread" value="${treadVal}" style="width:60px;text-align:center;">`}</td><td><input type="number" id="w_${pos.key}_psi" value="${w.pressure||''}" style="width:60px;text-align:center;"> <span style="font-size:11px;">PSI</span></td><td>${isSpare?'—':`<input type="number" step="0.1" id="w_${pos.key}_brakes" value="${brakesVal}" style="width:60px;text-align:center;">`}</td><td><input type="text" id="w_${pos.key}_note" value="${escHtml(noteVal)}" placeholder="${isSpare?'spare note':'notes'}" style="width:100%;min-width:100px;"></td></tr>`;
  });
  document.getElementById('tyreAdvisory').value=advisory||'';
}

function buildFindingsUI(section,items){
  const container=document.getElementById(section+'Rows'); container.innerHTML='';
  (items||[]).forEach(item=>addFindingRow(section,item));
}

function addFinding(section,item){ addFindingRow(section,item||{component:'',status:'CRITICAL',notes:''}); }
...
function addFindingRow(section,item){
  ...
  div.innerHTML=`<input type="text" placeholder="Component" value="${escHtml(item.component||'')}"><select><option value="CRITICAL" ${item.status==='CRITICAL'?'selected':''}>CRITICAL</option><option value="ADVISORY" ${item.status==='ADVISORY'?'selected':''}>ADVISORY</option><option value="OK" ${item.status==='OK'?'selected':''}>OK</option></select><input type="text" class="finding-notes" placeholder="Notes" value="${escHtml(item.notes||'')}"><button class="btn-remove" onclick="this.parentElement.remove()">×</button>`;
  container.appendChild(div);
}
...
function updateLabourTotal(){
  const rate=isCarGuys()?50:70;
  const rows=document.getElementById('labourRows').children;
  let total=0,count=0;
  for(const row of rows){ const hrs=parseFloat(row.querySelectorAll('input')[1]?.value)||0; if(hrs>0){total+=hrs*rate;count++;} }
  const bar=document.getElementById('labourTotal'); const amt=document.getElementById('labourTotalAmt');
...
function addAdditionalNote(item){
  const container=document.getElementById('additionalNotesRows');
...
function addChassisRow(){ const tbody=document.getElementById('chassisRows'); addChassisTableRow(tbody,'','unknown',''); }

function addChassisTableRow(tbody,label,status,notes){
  const tr=document.createElement('tr');
  tr.innerHTML=`<td><input type="text" value="${escHtml(label)}" placeholder="Component" style="width:100%;font-size:12px;padding:5px;"></td><td><select style="font-size:11px;padding:4px;width:100%;"><option value="unknown" ${status==='unknown'?'selected':''}>— ?</option><option value="OK" ${status==='OK'?'selected':''}>✓ OK</option><option value="ADVISORY" ${status==='ADVISORY'?'selected':''}>Advisory</option><option value="CRITICAL" ${status==='CRITICAL'?'selected':''}>Critical</option></select></td><td><input type="text" value="${escHtml(notes)}" placeholder="Notes" style="width:100%;font-size:12px;padding:5px;"></td><td><button class="btn-remove" onclick="this.closest('tr').remove()">×</button></td>`;
...
function setAllOk(){
  ...
  document.getElementById('flagsContainer').innerHTML='';
  toast('All unset items marked ✓ OK');
}
...
function collectData(){
  ...
  const data={ customerType:ct, reportType:rt, isCarGuys:cg, isCash:cash, isRepairOnly:repairOnly, rego:document.getElementById('rego').value.toUpperCase(), date:document.getElementById('jobDate').value||todayDate(), make:document.getElementById('vehicleMake').value, model:document.getElementById('vehicleModel').value, custName:document.getElementById('custName').value, custMobile:document.getElementById('custMobile').value, custEmail:document.getElementById('custEmail').value, odometer:document.getElementById('odometer').value, exterior:[], bonnet:[], wheels:[], tyreAdvisory:document.getElementById('tyreAdvisory')?document.getElementById('tyreAdvisory').value:'', findings:[], chassis:[], repairFindings:[], workCompleted:document.getElementById('workCompleted').value.split('\n').filter(Boolean), faultRect:document.getElementById('faultRect').value, additionalNotes:[], labour:[], stock:[], deduct:[], reimburse:[], privateParts:[], quickChecksDesc:document.getElementById('quickChecksDesc').value };
  EXTERIOR_ITEMS.forEach(item=>{ const s=document.getElementById(`ext_${item.key}_status`), n=document.getElementById(`ext_${item.key}_note`); if(s&&s.value!=='unknown') data.exterior.push({label:item.label,status:s.value,note:n?n.value:''}); });
  BONNET_ITEMS.forEach(item=>{ const s=document.getElementById(`bon_${item.key}_status`), n=document.getElementById(`bon_${item.key}_note`); if(s&&s.value!=='unknown') data.bonnet.push({label:item.label,status:s.value,note:n?n.value:''}); });
  const levNoteEl=document.getElementById('bonnetLevelsNoteText'); data.bonnetLevelsNote=levNoteEl?levNoteEl.value.trim():'';
  WHEEL_POSITIONS.forEach(pos=>{ const isSpare=pos.key==='spare'; const treadEl=document.getElementById(`w_${pos.key}_tread`), psiEl=document.getElementById(`w_${pos.key}_psi`), brakesEl=document.getElementById(`w_${pos.key}_brakes`), noteEl=document.getElementById(`w_${pos.key}_note`); const noteVal=noteEl?noteEl.value:''; const isShoes=(pos.key==='lr'||pos.key==='rr')&&noteVal.match(/shoe|drum/i); data.wheels.push({label:pos.label,isSpare,tread:treadEl?treadEl.value:null,pressure:psiEl?psiEl.value:null,brakes:brakesEl?brakesEl.value:null,note:noteVal,isShoes:!!isShoes}); });
  data.findingsText=document.getElementById('findingsText')?document.getElementById('findingsText').value.trim():'';
  data.faultRect=document.getElementById('faultRect')?document.getElementById('faultRect').value.trim():'';
  document.getElementById('chassisRows').querySelectorAll('tr').forEach(row=>{ const inputs=row.querySelectorAll('input'), sel=row.querySelector('select'); if(inputs[0]&&inputs[0].value&&sel&&sel.value!=='unknown') data.chassis.push({component:inputs[0].value,status:sel.value,notes:inputs[1]?inputs[1].value:''}); });
  Array.from(document.getElementById('repairFindingsRows').children).forEach(row=>{ const inputs=row.querySelectorAll('input,select'); if(inputs[0]&&inputs[0].value) data.repairFindings.push({component:inputs[0].value,status:inputs[1].value,notes:inputs[2].value}); });
...
function generatePDF(){
  ...
  // Save complete snapshot BEFORE generating PDF
  ...
  function sectionHeader(title){ return{text:title,bold:true,color:C.white,fillColor:C.navy,fontSize:10,margin:[4,4,4,4]}; }
  function colHeader(text,align='center'){ return{text,bold:true,color:C.white,fillColor:C.navyMid,fontSize:9,alignment:align,margin:[3,4,3,4]}; }
  function dataCell(text,opts={}){ return{text:String(text||''),fontSize:9,color:C.navy,margin:[3,3,3,3],...opts}; }
  function rowBg(i){ return i%2===0?C.white:C.grey; }

  if(!d.isRepairOnly&&d.exterior.length>0){
    const extBody=[[{text:'VEHICLE EXTERIOR',colSpan:3,...sectionHeader('VEHICLE EXTERIOR')},{},{}],[colHeader('ITEM','left'),colHeader('RESULT'),colHeader('NOTES','left')]];
    d.exterior.forEach((item,i)=>{ const badge=statusBadgePDF(item.status); extBody.push([{...dataCell(item.label,{bold:true}),fillColor:rowBg(i)},{text:badge.text,bold:true,fontSize:9,color:badge.color,fillColor:badge.fillColor,alignment:'center',margin:[3,3,3,3]},{...dataCell(item.note),fillColor:rowBg(i)}]); });
    content.push({table:{widths:[120,80,'*'],body:extBody},layout:{hLineWidth:()=>0.3,vLineWidth:()=>0,hLineColor:()=>C.grey},margin:[0,0,0,10]});
  }

  if(!d.isRepairOnly&&d.bonnet.length>0){
    const bonBody=[[{text:'UNDER BONNET',colSpan:3,...sectionHeader('UNDER BONNET')},{},{}],[colHeader('ITEM','left'),colHeader('RESULT'),colHeader('NOTES','left')]];
    d.bonnet.forEach((item,i)=>{ const badge=statusBadgePDF(item.status); bonBody.push([{...dataCell(item.label,{bold:true}),fillColor:rowBg(i)},{text:badge.text,bold:true,fontSize:9,color:badge.color,fillColor:badge.fillColor,alignment:'center',margin:[3,3,3,3]},{...dataCell(item.note),fillColor:rowBg(i)}]); });
    content.push({table:{widths:[120,80,'*'],body:bonBody},layout:{hLineWidth:()=>0.3,vLineWidth:()=>0,hLineColor:()=>C.grey},margin:[0,0,d.bonnetLevelsNote?2:10,0]});
    if(d.bonnetLevelsNote) content.push({text:`* ${d.bonnetLevelsNote}`,fontSize:8,italics:true,color:C.textMuted,margin:[4,0,0,10]});
  }

  if(!d.isRepairOnly&&d.wheels.length>0){
    const wtBody=[[{text:'BRAKES & TYRES',colSpan:5,...sectionHeader('BRAKES & TYRES')},{},{},{},{}],[colHeader('POSITION','left'),colHeader('TREAD DEPTH'),colHeader('PRESSURE'),colHeader('BRAKES'),colHeader('NOTES','left')]];
    d.wheels.forEach((w,i)=>{ const isSpare=w.isSpare; const treadVal=isSpare||w.tread===''||w.tread===null?'—':w.tread+'mm'; const treadC=isSpare?C.navy:treadColor(w.tread); const shoesFlag=w.isShoes; const brakeDisplay=isSpare||w.brakes===''||w.brakes===null?'—':w.brakes+'mm'+(shoesFlag?' (shoes)':''); const brakeC=isSpare?C.navy:brakeColor(w.brakes,shoesFlag); const autoNote=isSpare?'':brakeNote(w.brakes,shoesFlag); const cleanNote=(w.note||'').replace(/\(?\s*shoes?\s*\)?/gi,'').trim(); const noteText=[cleanNote,autoNote].filter(Boolean).join(' · '); wtBody.push([{...dataCell(w.label,{bold:true}),fillColor:rowBg(i)},{text:treadVal,bold:true,color:treadC,fontSize:9,alignment:'center',margin:[3,3,3,3],fillColor:rowBg(i)},{...dataCell(w.pressure?w.pressure+' PSI':'—'),alignment:'center',fillColor:rowBg(i)},{text:brakeDisplay,bold:true,color:brakeC,fontSize:9,alignment:'center',margin:[3,3,3,3],fillColor:rowBg(i)},{...dataCell(noteText),fillColor:rowBg(i)}]); });
    ...
    content.push({table:{widths:[80,60,55,55,'*'],body:wtBody},layout:{hLineWidth:()=>0.3,vLineWidth:()=>0,hLineColor:()=>C.grey},margin:[0,0,0,10]});
  }

  if(!d.isRepairOnly&&d.findingsText){ const findLines=d.findingsText.split('\n').filter(Boolean); const fStack=[{text:'INSPECTION FINDINGS',bold:true,color:C.white,fillColor:C.navy,fontSize:10,margin:[4,4,4,4]}]; findLines.forEach(line=>fStack.push({text:'• '+line,fontSize:9,color:C.navy,margin:[4,2,4,2]})); content.push({table:{widths:['*'],body:[[{stack:fStack}]]},layout:{hLineWidth:()=>0,vLineWidth:()=>0},margin:[0,0,0,10]}); }

  if(!d.isRepairOnly&&d.chassis.length>0){
    const cBody=[[{text:'CHASSIS & BODY',colSpan:3,...sectionHeader('CHASSIS & BODY')},{},{}],[colHeader('COMPONENT','left'),colHeader('STATUS'),colHeader('NOTES','left')]];
    d.chassis.forEach((f,i)=>{ const badge=statusBadgePDF(f.status); cBody.push([{...dataCell(f.component,{bold:true}),fillColor:rowBg(i)},{text:badge.text,bold:true,fontSize:9,color:badge.color,fillColor:badge.fillColor,alignment:'center',margin:[3,3,3,3]},{...dataCell(f.notes),fillColor:rowBg(i)}]); });
    content.push({table:{widths:[160,70,'*'],body:cBody},layout:{hLineWidth:()=>0.3,vLineWidth:()=>0,hLineColor:()=>C.grey},margin:[0,0,0,10]});
  }

  if(d.isRepairOnly&&d.repairFindings.length>0){
    const rBody=[[{text:'REPAIR FINDINGS',colSpan:3,...sectionHeader('REPAIR FINDINGS')},{},{}],[colHeader('COMPONENT','left'),colHeader('STATUS'),colHeader('NOTES','left')]];
    d.repairFindings.forEach((f,i)=>{ const badge=statusBadgePDF(f.status); rBody.push([{...dataCell(f.component,{bold:true}),fillColor:rowBg(i)},{text:badge.text,bold:true,fontSize:9,color:badge.color,fillColor:badge.fillColor,alignment:'center',margin:[3,3,3,3]},{...dataCell(f.notes),fillColor:rowBg(i)}]); });
    content.push({table:{widths:[120,70,'*'],body:rBody},layout:{hLineWidth:()=>0.3,vLineWidth:()=>0,hLineColor:()=>C.grey},margin:[0,0,0,10]});
  }
  ...
  content.push({table:{widths:['*'],body:[[{stack:wcStack}]]},layout:{hLineWidth:()=>0,vLineWidth:()=>0},margin:[0,0,0,10]});
  ...
  if(hasStock||hasDeduct||hasReim||hasPrivParts){
    ...
    if(hasStock){ const sBody=[[{text:'A) Car Guys Stock',colSpan:3,bold:true,fontSize:9,color:C.navy,fillColor:'#e8eef8',margin:[4,4,4,4]},{},{}],[colHeader('ITEM / PART NO.','left'),colHeader('QTY'),colHeader('NOTES','left')]]; d.stock.forEach((s,i)=>sBody.push([{...dataCell(s.item),fillColor:rowBg(i)},{...dataCell(s.qty),alignment:'center',fillColor:rowBg(i)},{...dataCell('Deduct from stock'),fillColor:rowBg(i)}])); content.push({table:{widths:['*',60,120],body:sBody},layout:{hLineWidth:()=>0.3,vLineWidth:()=>0,hLineColor:()=>C.grey},margin:[0,4,0,6]}); }
    if(hasDeduct){ const deductTotal=d.deduct.reduce((s,r)=>s+r.cost,0); const dBody=[[{text:'B) Deductions (Car Guys stock → private customer)',colSpan:6,bold:true,fontSize:9,color:C.navy,fillColor:'#e8eef8',margin:[4,4,4,4]},{},{},{},{},{}],[colHeader('ITEM','left'),colHeader('PART NO.'),colHeader('SUPPLIER'),colHeader('QTY'),colHeader('COST (ex GST)'),colHeader('')]]; d.deduct.forEach((r,i)=>dBody.push([{...dataCell(r.item),fillColor:rowBg(i)},{...dataCell(r.partNo),alignment:'center',fillColor:rowBg(i)},{...dataCell(r.supplier),fillColor:rowBg(i)},{...dataCell(r.qty),alignment:'center',fillColor:rowBg(i)},{...dataCell('$'+r.cost.toFixed(2)),alignment:'right',fillColor:rowBg(i)},{...dataCell(''),fillColor:rowBg(i)}])); dBody.push([{text:'Deductions Total',colSpan:5,bold:true,fontSize:9,color:C.red,alignment:'right',margin:[3,4,3,4]},{},{},{},{},{text:'–$'+deductTotal.toFixed(2),bold:true,fontSize:9,color:C.red,alignment:'right',margin:[3,4,3,4]}]); content.push({table:{widths:['*',60,70,40,80,60],body:dBody},layout:{hLineWidth:()=>0.3,vLineWidth:()=>0,hLineColor:()=>C.grey},margin:[0,4,0,6]}); }
    if(hasReim){ const reimTotal=d.reimburse.reduce((s,r)=>s+r.cost,0); const rBody=[[{text:'C) Parts – Reimbursement',colSpan:6,bold:true,fontSize:9,color:C.navy,fillColor:'#e8eef8',margin:[4,4,4,4]},{},{},{},{},{}],[colHeader('ITEM','left'),colHeader('PART NO.'),colHeader('SUPPLIER'),colHeader('QTY'),colHeader('COST (ex GST)'),colHeader('')]]; d.reimburse.forEach((r,i)=>rBody.push([{...dataCell(r.item),fillColor:rowBg(i)},{...dataCell(r.partNo),alignment:'center',fillColor:rowBg(i)},{...dataCell(r.supplier),fillColor:rowBg(i)},{...dataCell(r.qty),alignment:'center',fillColor:rowBg(i)},{...dataCell('$'+r.cost.toFixed(2)),alignment:'right',fillColor:rowBg(i)},{...dataCell(''),fillColor:rowBg(i)}])); rBody.push([{text:'Parts Total',colSpan:5,bold:true,fontSize:9,color:C.navy,alignment:'right',margin:[3,4,3,4]},{},{},{},{},{text:'$'+reimTotal.toFixed(2),bold:true,fontSize:9,color:C.navy,alignment:'right',margin:[3,4,3,4]}]); content.push({table:{widths:['*',60,70,40,80,60],body:rBody},layout:{hLineWidth:()=>0.3,vLineWidth:()=>0,hLineColor:()=>C.grey},margin:[0,4,0,6]}); }
    if(hasPrivParts){ const ppTotal=d.privateParts.reduce((s,r)=>s+r.cost,0); const pBody=[[{text:'Parts (at cost)',colSpan:5,bold:true,fontSize:9,color:C.navy,fillColor:'#e8eef8',margin:[4,4,4,4]},{},{},{},{}],[colHeader('ITEM','left'),colHeader('SOURCE'),colHeader('QTY'),colHeader('COST'),colHeader('NOTES')]]; d.privateParts.forEach((r,i)=>pBody.push([{...dataCell(r.item),fillColor:rowBg(i)},{...dataCell(r.source),fillColor:rowBg(i)},{...dataCell(r.qty),alignment:'center',fillColor:rowBg(i)},{...dataCell('$'+r.cost.toFixed(2)),alignment:'right',fillColor:rowBg(i)},{...dataCell('At cost'),fillColor:rowBg(i)}])); pBody.push([{text:'Parts Total',colSpan:4,bold:true,fontSize:9,color:C.navy,alignment:'right',margin:[3,4,3,4]},{},{},{},{text:'$'+ppTotal.toFixed(2),bold:true,fontSize:9,color:C.navy,alignment:'right',margin:[3,4,3,4]}]); content.push({table:{widths:['*',80,40,70,70],body:pBody},layout:{hLineWidth:()=>0.3,vLineWidth:()=>0,hLineColor:()=>C.grey},margin:[0,4,0,6]}); }
  }
  ...
  const invBody=[[{text:invoiceTitle,colSpan:4,bold:true,color:C.white,fillColor:C.navy,fontSize:12,margin:[4,6,4,6]},{},{},{}],[{text:`To: ${toLine}`,colSpan:2,fontSize:9,color:C.navy,margin:[4,4,4,4]},{text:''},{},{text:`From: ${fromLine}`,fontSize:9,color:C.navy,alignment:'right',margin:[4,4,4,4]}],[colHeader('DESCRIPTION','left'),colHeader('HRS'),colHeader('RATE'),colHeader('AMOUNT (ex GST)','right')]];
  d.labour.forEach((l,i)=>{ invBody.push([{...dataCell(l.desc),fillColor:rowBg(i)},{...dataCell(l.hrs||''),alignment:'center',fillColor:rowBg(i)},{...dataCell(l.hrs?`$${rate}.00/hr`:''),alignment:'center',fillColor:rowBg(i)},{...dataCell(l.hrs?'$'+(l.hrs*rate).toFixed(2):''),alignment:'right',fillColor:rowBg(i)}]); if(i===0&&d.quickChecksDesc) invBody.push([{text:d.quickChecksDesc,colSpan:4,fontSize:8,color:'#b07000',italics:true,fillColor:C.amberLight,margin:[8,2,4,2]},{},{},{}]); });
  ...
  if(reimTotal>0){ invBody.push([{...dataCell('Parts – reimbursement (see Parts & Inventory)'),fillColor:rowBg(rowIdx)},{...dataCell('—'),alignment:'center',fillColor:rowBg(rowIdx)},{...dataCell('—'),alignment:'center',fillColor:rowBg(rowIdx)},{...dataCell('$'+reimTotal.toFixed(2)),alignment:'right',fillColor:rowBg(rowIdx)}]); rowIdx++; }
  if(ppTotal>0){ invBody.push([{...dataCell('Parts – at cost (see Parts & Inventory)'),fillColor:rowBg(rowIdx)},{...dataCell(''),alignment:'center',fillColor:rowBg(rowIdx)},{...dataCell('At cost'),alignment:'center',fillColor:rowBg(rowIdx)},{...dataCell('$'+ppTotal.toFixed(2)),alignment:'right',fillColor:rowBg(rowIdx)}]); rowIdx++; }
  if(deductTotal>0){ invBody.push([{...dataCell('Less: Car Guys stock parts – repayable deduction (see Parts & Inventory)'),color:C.red,fillColor:rowBg(rowIdx)},{...dataCell('—'),alignment:'center',fillColor:rowBg(rowIdx)},{...dataCell('—'),alignment:'center',fillColor:rowBg(rowIdx)},{text:'–$'+deductTotal.toFixed(2),bold:true,color:C.red,fontSize:9,alignment:'right',margin:[3,3,3,3],fillColor:rowBg(rowIdx)}]); rowIdx++; }
  ...
  content.push({table:{widths:['*',40,70,80],body:invBody},layout:{hLineWidth:()=>0.3,vLineWidth:()=>0,hLineColor:()=>C.grey},margin:[0,0,0,8]});
  ...
  // Save snapshot with invoice total
...
function autosave(){
  try {
    const snap=collectCompleteSnapshot();
    snap.savedAt=new Date().toLocaleTimeString();
    localStorage.setItem('jw_draft', JSON.stringify(snap));
    const el=document.getElementById('savedIndicator');
    if(el) el.textContent='Saved '+snap.savedAt;
  } catch(e) {}
}

function loadSaved(){
  try {
    const raw=localStorage.getItem('jw_draft');
    if(!raw){ toast('No saved draft found.'); return; }
    const snap=JSON.parse(raw);
    // Use restoreJob-style logic for complete snapshot drafts
    if(snap.jobId||snap.exterior){
      document.getElementById('customerType').value=snap.customerType||'carguysinspection';
      document.getElementById('reportType').value=snap.reportType||'full';
      document.getElementById('rego').value=snap.rego||'';
      document.getElementById('jobDate').value=snap.date||'';
      ...
      document.getElementById('notesInput').value=snap.notes||'';
      onCustomerTypeChange();
      toast(`↩ Draft restored (${snap.savedAt||''})`);
    } else {
      // Legacy basic draft
      document.getElementById('customerType').value=snap.customerType||'carguysinspection';
      document.getElementById('reportType').value=snap.reportType||'full';
...
function clearAll(){
  ...
  localStorage.removeItem('jw_draft');
  ['notesInput','rego','jobDate','vehicleMake','vehicleModel','custName','custMobile','custEmail','odometer'].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=''; });
...
function saveCustomer(name,mobile,email){
  if(!name) return;
  let customers=getCustomers();
  const idx=customers.findIndex(c=>c.name.toLowerCase()===name.toLowerCase());
  const record={name,mobile,email,lastSeen:new Date().toLocaleDateString('en-AU')};
  if(idx>=0) customers[idx]=record; else customers.unshift(record);
  customers=customers.slice(0,100);
  localStorage.setItem('jw_customers',JSON.stringify(customers));
}

function customerAutocomplete(input){
  const val=input.value.toLowerCase(); const box=document.getElementById('custAutocomplete');
  if(!val||val.length<2){ box.style.display='none'; return; }
  const matches=getCustomers().filter(c=>c.name.toLowerCase().startsWith(val));
  if(!matches.length){ box.style.display='none'; return; }
  box.innerHTML=matches.slice(0,5).map(c=>`<div onclick="fillCustomer('${c.name.replace(/'/g,"\\'")}','${(c.mobile||'').replace(/'/g,"\\'")}','${(c.email||'').replace(/'/g,"\\'")}' )" style="padding:10px 14px;cursor:pointer;border-bottom:1px solid #eee;font-size:13px;" onmouseover="this.style.background='#f4f6fa'" onmouseout="this.style.background='white'"><strong>${c.name}</strong><br><span style="color:#6b7a99;font-size:11px;">${c.mobile||''} ${c.email?'· '+c.email:''}</span></div>`).join('');
  box.style.display='block';
}

function fillCustomer(name,mobile,email){
...
// ═══════════════════════════════════════════════════════════
//  LEGACY HISTORY (kept for backward compat)
// ═══════════════════════════════════════════════════════════
function getHistory(){ try{ return JSON.parse(localStorage.getItem('jw_history')||'[]'); }catch(e){ return []; } }

function saveToHistory(d,total){
  let history=getHistory();
  history.unshift({ rego:d.rego, make:d.make, model:d.model, date:d.date, reportType:d.reportType, customerType:d.customerType, custName:d.custName||'', total:total, generatedAt:new Date().toLocaleString('en-AU') });
  history=history.slice(0,200);
  localStorage.setItem('jw_history',JSON.stringify(history));
}
...
function exportBackup(){
  const backup={ exportedAt:new Date().toISOString(), version:2, customers:getCustomers(), history:getHistory(), jobs:getJobs(), draft:JSON.parse(localStorage.getItem('jw_draft')||'null') };
  const blob=new Blob([JSON.stringify(backup,null,2)],{type:'application/json'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a'); a.href=url; a.download=`JW_Reports_Backup_${new Date().toLocaleDateString('en-AU').replace(/\//g,'-')}.json`; a.click();
...
function importBackup(event){
  ...
  reader.onload=function(e){
    try{
      ...
      if(!backup.version) throw new Error('Invalid backup');
      if(backup.customers) localStorage.setItem('jw_customers',JSON.stringify(backup.customers));
      if(backup.history)   localStorage.setItem('jw_history',JSON.stringify(backup.history));
      if(backup.jobs)      localStorage.setItem('jw_jobs',JSON.stringify(backup.jobs));
      if(backup.draft)     localStorage.setItem('jw_draft',JSON.stringify(backup.draft));
      toast(`✓ Restored: ${(backup.customers||[]).length} customers, ${(backup.jobs||[]).length} jobs`);
    }catch(err){ toast('Error reading backup file.'); }
    event.target.value='';
  };
...
document.addEventListener('click',function(e){
  if(!e.target.closest('#dropdownMenu')&&!e.target.closest('#menuBtn')) document.getElementById('dropdownMenu').style.display='none';
  if(!e.target.closest('#custAutocomplete')&&e.target.id!=='custName'){ const box=document.getElementById('custAutocomplete'); if(box) box.style.display='none'; }
});
